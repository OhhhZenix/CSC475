...
fun EditExerciseLogScreen(
    ...
    val exercises = remember { mutableStateListOf<Pair<String, Int>>() }
    val selectedExerciseName = remember { mutableStateOf("") }
    val selectedExerciseId = remember { mutableIntStateOf(0) }
    ...
    LaunchedEffect(Unit) {
        exerciseCatalogDao.getAllExercise().forEach {
            exercises.add(Pair(it.name, it.id))
        }
        exerciseLogDao.getLog(selectedExerciseLog.intValue).let {
            pickedDate = it.dateTime.toLocalDate()
            pickedTime = it.dateTime.toLocalTime()
            quantity.value = it.quantity.toString()
            selectedExerciseId.intValue = it.exerciseId
            selectedExerciseName.value =
                exerciseCatalogDao.getExercise(selectedExerciseId.intValue)?.name ?: ""
        }
    }
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            Text(
                "(Selected Exercise Log ID: ${selectedExerciseLog.intValue})",
            ...
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    value = selectedExerciseName.value,
                    onValueChange = {},
                    label = { Text("Exercise") },
                    readOnly = true,
                ...
                ExposedDropdownMenu(expanded = isExpanded.value,
                    ...
                    exercises.forEach {
                        DropdownMenuItem(text = { Text(it.first) }, onClick = {
                            selectedExerciseName.value = it.first
                            selectedExerciseId.intValue = it.second
            ...
            Button(
                onClick = {
                    if (selectedExerciseName.value.isEmpty()) {
                        Toast.makeText(
                            context, "A exercise is not selected. Try again.", Toast.LENGTH_SHORT
                        ).show()
                        ...
                        coroutineScope.launch {
                            exerciseLogDao.updateLog(
                                id = selectedExerciseLog.intValue,
                                dateTime = pickedDate.atTime(pickedTime),
                                exerciseId = selectedExerciseId.intValue,
                                quantity = quantity.value.toDouble()
                            )
                        }
                        navController.navigate(AppScreen.ExerciseLog.name)
                    }
                }, modifier = Modifier
                    .fillMaxWidth()
                    .padding(start = 8.dp, end = 8.dp)
            ...
            Button(
                onClick = {
                    coroutineScope.launch {
                        exerciseLogDao.removeLog(selectedExerciseLog.intValue)
                    }
                    navController.navigate(AppScreen.ExerciseLog.name)
                },
                modifier = Modifier
